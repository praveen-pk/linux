...
void mshv_isr(void)
{
	...
	if (handled) {
		...
		if (msg->header.message_flags.msg_pending)
			hv_set_register(HV_SYN_REG_EOM, 0);
...
int mshv_synic_init(unsigned int cpu)
{
	...
	simp.as_uint64 = hv_get_register(HV_SYN_REG_SIMP);
	...
	*msg_page = memremap(simp.base_simp_gpa << HV_HYP_PAGE_SHIFT,
	...
	hv_set_register(HV_SYN_REG_SIMP, simp.as_uint64);

	/* Setup the Synic's event flags page */
	siefp.as_uint64 = hv_get_register(HV_SYN_REG_SIEFP);
	...
	*event_flags_page = memremap(siefp.base_siefp_gpa << PAGE_SHIFT,
	...
	hv_set_register(HV_SYN_REG_SIEFP, siefp.as_uint64);

	/* Setup the Synic's event ring page */
	sirbp.as_uint64 = hv_get_register(HV_SYN_REG_SIRBP);
	...
	*event_ring_page = memremap(sirbp.base_sirbp_gpa << PAGE_SHIFT,
	...
	hv_set_register(HV_SYN_REG_SIRBP, sirbp.as_uint64);
	...
	sint.as_intercept = 1;
	sint.auto_eoi = hv_recommend_using_aeoi();
	hv_set_register(HV_SYN_REG_SINT0 + HV_SYNIC_DOORBELL_SINT_INDEX,
			sint.as_uint64);
#endif

	/* Enable global synic bit */
	sctrl.as_uint64 = hv_get_register(HV_SYN_REG_SCONTROL);
	sctrl.enable = 1;
	hv_set_register(HV_SYN_REG_SCONTROL, sctrl.as_uint64);
...
int mshv_synic_cleanup(unsigned int cpu)
{
	...
	sint.as_uint64 = hv_get_register(HV_SYN_REG_SINT0 + HV_SYNIC_INTERCEPTION_SINT_INDEX);
	sint.masked = true;
	hv_set_register(HV_SYN_REG_SINT0 + HV_SYNIC_INTERCEPTION_SINT_INDEX,
			sint.as_uint64);

	/* Disable Doorbell SINT */
	sint.as_uint64 = hv_get_register(HV_SYN_REG_SINT0 + HV_SYNIC_DOORBELL_SINT_INDEX);
	sint.masked = true;
	hv_set_register(HV_SYN_REG_SINT0 + HV_SYNIC_DOORBELL_SINT_INDEX,
			sint.as_uint64);
