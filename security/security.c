			       ...
			       void **buffer, bool alloc)
{
	...
	lsm_for_each_hook(scall, inode_getsecurity) {
		rc = scall->hl->hook.inode_getsecurity(idmap, inode, name,
						       buffer, alloc);
			       ...
			       const void *value, size_t size, int flags)
{
	...
	lsm_for_each_hook(scall, inode_setsecurity) {
		rc = scall->hl->hook.inode_setsecurity(inode, name, value, size,
						       flags);
...
int security_inode_setintegrity(const struct inode *inode,
				...
				size_t size)
{
	return call_int_hook(inode_setintegrity, 0, inode, type, value, size);
}
...
int security_bdev_setintegrity(struct block_device *bdev,
			       ...
			       size_t size)
{
	return call_int_hook(bdev_setintegrity, 0, bdev, type, value, size);
}
